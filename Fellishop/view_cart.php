...
<div class="container">
	<nav class="navbar navbar-default">
	  <div class="container-fluid_produk">
			...
			<table class="table table-bordered table-striped">
				...
				<tbody>
					<?php
						...
						$sql = "SELECT * FROM products WHERE id_produk IN (".implode(',',$_SESSION['cart']).")";
						$query = $conn->query($sql);
							while($row = $query->fetch_assoc()){
								?>
								<tr>
									<td>
										<a href="delete_item.php?id_produk=<?php echo $row['id_produk']; ?>&index=<?php echo $index; ?>" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></a>
									</td>
									<td><?php echo $row['nama_produk']; ?></td>
									<td><?php echo number_format($row['harga_produk'], 2); ?></td>
									<input type="hid_produkden" name="indexes[]" value="<?php echo $index; ?>">
									<td><input type="text" class="form-control" value="<?php echo $_SESSION['qty_array'][$index]; ?>" name="qty_<?php echo $index; ?>"></td>
									<td><?php echo number_format($_SESSION['qty_array'][$index]*$row['harga_produk'], 2); ?></td>
									<?php $total += $_SESSION['qty_array'][$index]*$row['harga_produk']; ?>
								</tr>
								<?php
								$index ++;
							}
